...
lmerTest::lmer(dnc ~ phase*PlotTreatmentStatus + (1|site/plot), data = diffs) -> fit
em <- emmeans(fit, ~ PlotTreatmentStatus | phase)
m5 <- pairs(em) |> broom::tidy() |> mutate(response = "Native Cover")

lmerTest::lmer(dnr ~ phase*PlotTreatmentStatus + (1|site/plot), data = diffs) -> fit
em <- emmeans(fit, ~ PlotTreatmentStatus | phase)
...
contrasts <- bind_rows(m1, m2, m3, m4, m5, m6) |>
  ...
  mutate(sig = ifelse(p.value < 0.05, "*", ""),
         position = c(30,30,30,20,15,15,14,12,11,.2,.2,.2,25,25,25,11,11,11)) 
...
ggsave('out/figure_3_cover_changes.png', width =10, height = 8, bg='white')
